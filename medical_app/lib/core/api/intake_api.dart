...
class PatientIntake {
  ...
  factory PatientIntake.fromJson(Map<String, dynamic> json) {
    ...
    final questions =
        (content['questions'] as List?)?.cast<Map<String, dynamic>>() ??
        const [];
...
Future<PatientIntake?> fetchPatientIntake(
  String patientId,
  String accessToken,
) async {
  ...
  if (response.statusCode != 200) {
    ...
    // ignore: avoid_print
    print(
      '❗ fetchPatientIntake ${response.statusCode} exception: ${match?.group(1) ?? body}',
    );
...
class QrPatientSummary {
  ...
  factory QrPatientSummary.fromJson(Map<String, dynamic> json) {
    ...
    final content = json['intake_form'] as Map<String, dynamic>?;
    final questions =
        (content?['questions'] as List?)?.cast<Map<String, dynamic>>() ??
        const [];
    ...
    return QrPatientSummary(
      ...
      intakeStatus: content?['status'] as String?,
    );
  }
}
...
/// 토큰이 만료/무효면 404.
Future<QrPatientSummary?> fetchQrPatientSummary(
  String token,
  String accessToken,
) async {
  ...
  if (response.statusCode != 200) {
    debugPrint('[IntakeQR] response statusCode=${response.statusCode}');
  ...
  try {
    final body =
        jsonDecode(utf8.decode(response.bodyBytes)) as Map<String, dynamic>;
    final intakeForm = body['intake_form'];
    final intakeStatus = intakeForm is Map<String, dynamic>
        ? intakeForm['status']
        : null;
    debugPrint(
      '[IntakeQR] intakeFormPresent=${intakeForm != null} '
      'intakeStatus=$intakeStatus',
    );
    return QrPatientSummary.fromJson(body);
  } catch (e) {
    debugPrint('[IntakeQR] response parse failed: ${e.runtimeType}');
